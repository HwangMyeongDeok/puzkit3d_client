...
export default function CheckoutPage() {
  ...
  const onSubmit = (data: CheckoutFormValues) => {
    ...
    if (isPartnerMode) {
      console.log('Quote request submitted:', { ...data, items: selectedItems, subtotal });
      setTimeout(() => {
      ...
      console.log('Order submitted:', { ...data, items: selectedItems, total });
      setTimeout(() => {
      ...
      {isPartnerMode && (
        <div className="border-info/30 bg-info/5 text-info mb-6 flex items-start gap-3 rounded-xl border px-5 py-3 text-sm">
          ...
          <p className="font-medium">
            Giá cuối cùng sẽ được Staff xác nhận dựa trên tỷ giá và phí vận chuyển thực tế. Sau khi
            gửi yêu cầu, Staff sẽ liên hệ bạn trong vòng 24h.
          </p>
        </div>
      )}
              ...
              >
                {isPartnerMode ? (
                  <>
                    ...
                    {form.formState.isSubmitting ? 'Đang gửi...' : 'Xác nhận yêu cầu'}
                  </>
                ) : (
                  <>
