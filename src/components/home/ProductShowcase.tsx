...
// Map màu cho các level độ khó (Đổi class cho khớp với ProductCard của ông)
...
export default function ProductShowcase() {
  ...
  const [selectedCapabilities, setSelectedCapabilities] = useState<string[]>([]); // Array cho Checkbox

  // Reset số lượng hiển thị về 8 mỗi khi đổi filter
  useEffect(() => {
    setVisibleCount(8);
  }, [difficultyLevel, materialSlug, assemblyMethodSlug, topicSlug, selectedCapabilities]);

  // 2. Lấy dữ liệu danh mục cho bộ lọc
  ...
  // Nối mảng capabilities thành chuỗi "cap-1,cap-2" gửi lên BE
  const capabilitySlugs =
    selectedCapabilities.length > 0 ? selectedCapabilities.join(',') : undefined;

  // 3. Fetch Products kèm params lọc
  ...
  // Toggle Checkbox cho Capabilities
  ...
  return (
    <div className="flex w-full flex-col gap-8 px-4 md:flex-row md:items-start lg:px-0">
      {/* ================= SIDEBAR: FILTER ================= */}
      {/* Thêm max-h-screen và overflow-y-auto để thanh cuộn dọc cho sidebar nếu nó quá dài */}
      <aside className="custom-scrollbar max-h-[calc(100vh-100px)] w-full shrink-0 flex-col gap-6 overflow-y-auto pr-2 md:sticky md:top-24 md:flex md:w-[260px]">
        <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          {/* Lọc: Độ khó (Badges/Pills màu sắc) */}
          <div className="mb-6">
                ...
                return (
                  ...
                  >
                    <input
                      type="radio"
                      name="difficulty"
                      className="hidden" // Ẩn nút radio đi, chỉ bấm vào cái badge
                      checked={isSelected}
          ...
          {/* Lọc: Capabilities (Cho phép chọn NHIỀU - Checkbox) */}
          <div>
            <h4 className="mb-3 text-xs font-semibold tracking-wider text-slate-500 uppercase">
              Capabilities
            </h4>
